...
=============================================================================*/
    ...
    */
    ...
    地里面经总结
    =============================================================================*/
/* git commit的题，也是面经题。第一问给一个commit（node），

    BFS输出所有commits(nodes)。
    第二问，两个commits （nodes），找到他们的最近的公共parent，就是先BFS一个，

    然后用map记录下其各个parent到这个commit(node)的距离，

    然后BFS第二个commit(node)，碰到在map里的node，
    就算一个总距离，然后更新最短距离和的点，最后最短距离和的点就是结果了，写完面试官也表示很满意。
    这个注意解释下BFS的复杂度为什么是O（V+E），
    ...
    <C> 是git commit，follow－
    up是个lowest common
    anscester，写第一问的时候竟然脑残出了个小bug，
...
<J>现在更新一下input，说的更清楚一点，
    第一题第一问input是一个parent pointer的node，
